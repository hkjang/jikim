...
// Event names. Each one maps to a settings switch in config.go so an
// administrator can silence a kind of mail without turning the relay off.
const (
...
// Validate reports why the configuration cannot send anything. It is exported
// so the settings handler can refuse to turn mail on with a relay it could
// never reach, instead of failing quietly on the first notification.
func (c Config) Validate() error {
	if strings.TrimSpace(c.Host) == "" {
		return fmt.Errorf("%w: mail.smtp_host가 필요합니다", ErrInvalid)
	}
	if c.Port < 1 || c.Port > 65535 {
		return fmt.Errorf("%w: mail.smtp_port는 1~65535여야 합니다", ErrInvalid)
	}
...
func Deliver(ctx context.Context, config Config, message Message) error {
	...
	if strings.TrimSpace(message.To) == "" {
		return fmt.Errorf("%w: 받는 사람이 필요합니다", ErrInvalid)
	}
...
func dial(ctx context.Context, config Config) (*smtp.Client, error) {
	...
	// The relay must answer every step within the same budget as the dial, or
	// a half-open relay would hold a background goroutine forever.
