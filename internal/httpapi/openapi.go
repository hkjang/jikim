...
func (s *Server) openAPI(w http.ResponseWriter, _ *http.Request) {
	...
	for _, endpoint := range []struct {
		...
		{"/api/v1/tracking/csp-report", "post", "브라우저 CSP 위반 신고 수신", false},
		{"/api/v1/tracking/violations", "get", "방문 추적 CSP 차단 출처 목록", true},
		{"/api/v1/tracking/violations", "delete", "방문 추적 CSP 차단 기록 삭제", true},
		{"/api/v1/tracking/violations/allow", "post", "차단된 출처를 허용 목록에 추가", true},
