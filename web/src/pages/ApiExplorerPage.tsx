...
import { Braces, CheckCircle2, Copy, Play, ShieldCheck, Terminal } from 'lucide-react';
...
export function ApiExplorerPage() {
  ...
  const curl = useMemo(() => `curl --request ${method} \\\n  --header "X-Vault-Token: $JIKIM_TOKEN"${!['GET', 'HEAD', 'DELETE', 'LIST'].includes(method) ? ` \\\n  --header "Content-Type: application/json" \\\n  --data '${body.replaceAll("'", "'\\''")}'` : ''} \\\n  "http://jikim.local:8080${path}"`, [method, path, body]);
  return <>
    <PageHeader eyebrow="Developer tools" title="API 탐색기" description="현재 세션 권한으로 Jikim REST API와 구현된 OpenBao 호환 API를 직접 확인합니다." />
  ...
  "id": 1,
  "method": "tools/list"
}`}</pre></Tabs.Panel></Tabs></Card><Card className="surface" radius="lg" p="xl" mt="lg"><Group mb="md"><CheckCircle2 color="#11b5ae" /><Title order={2} fz="lg">호환 프로파일</Title></Group><Stack gap="xs"><Text>• OpenBao 2.6.1 기준</Text><Text>• KV v2 일부 data·metadata 작업</Text><Text>• Token lookup/create/revoke 일부</Text><Text>• Transit encrypt/decrypt 일부</Text><Text>• 전체 OpenBao API 호환을 의미하지 않음</Text></Stack></Card></Grid.Col></Grid>
  </>;
}
