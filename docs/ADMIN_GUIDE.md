...
이 문서는 jikim `v0.2.13`을 설치하고 지키는 사람을 위한 것입니다. 화면 캡처는 모두
...
| 구성 요소 | 필수 | 제공 방식 | 하는 일 |
| --- | --- | --- | --- |
| `jikim` 컨테이너 | 필수 | 릴리스 이미지 `jikim:v0.2.13` | Go 서버 하나가 관리 API, OpenBao 제한 호환 API, MCP, React 정적 자산을 모두 제공 |
...
```bash
sha256sum --check jikim-v0.2.13.tar.gz.sha256
docker load --input jikim-v0.2.13.tar.gz
docker image inspect jikim:v0.2.13
```
...
- `GET /v1/sys/health`는 OpenBao 클라이언트와 Load Balancer가 쓰는 경로입니다. `v0.2.13`은
...
| 방문 추적 | 사용 여부, 제공자(Momento·GA4·GTM·Matomo·직접 붙여넣기), 수집기 주소와 사이트 ID, 같은 오리진 프록시, 삽입 위치, 추가 허용 출처, 관리 화면 포함, 수집기 내부 HTTP 허용, 정책이 차단한 출처 목록 |
| 메일 | SMTP 릴레이 주소·포트·보안·인증(선택), 보내는 사람, 앱 주소, 이벤트별 스위치, 시험 발송, 발송 기록 |
...
`v0.2.13`에서 설정할 수 있는 것은 워크플로 사용 여부, 적용 작업(시크릿 생성·변경, 시크릿 폐기),
...
**방문 추적** 탭에서 화면에 방문 추적 스크립트를 붙입니다. 기본값은 **꺼짐**이며, 켜기 전까지
어떤 화면에도 스니펫이 들어가지 않고 응답 헤더도 이전 릴리스와 같습니다. 재배포 없이 화면에서
켜고 끄며, 값은 다른 관리 설정과 같이 PostgreSQL의 `settings` 테이블(`tracking` 키)에
저장됩니다.

![서비스 관리 설정 — 방문 추적 탭. CSP 안내, 제공자 선택, Momento 수집기 주소·사이트 ID·환경, 같은 오리진 프록시 스위치](screenshots/admin-settings-tracking.png)

| 항목 | 뜻 |
| --- | --- |
| 방문 추적 사용 | 기본 OFF. 저장 후 새로 여는 화면부터 스니펫이 들어갑니다. |
| 제공자 | `momento`(첫 자리, 권장) · `ga4` · `gtm` · `matomo` · `custom`(직접 붙여넣기) |
| Momento 수집기 주소 · 사이트 ID · 환경 이름 | 사내 Momento 수집기의 오리진, 사이트 id, `data-environment` 값(기본 `prd`) |
| 같은 오리진 프록시 사용 | 기본 ON. jikim이 `/momento/*`를 수집기로 넘기고 스니펫에 `data-endpoint="/momento"`를 줍니다. |
| 측정 ID · 컨테이너 ID | GA4·GTM의 id |
| Matomo 주소 · 사이트 ID | Matomo 설치 주소와 사이트 id |
| 추적 스니펫 | 직접 붙여넣는 `<script>` 태그. **8KB**를 넘으면 `400`으로 거부합니다. |
| 추가 허용 출처 | 스니펫에서 자동으로 읽지 못한 출처를 쉼표로 구분해 적는 자리 |
| 관리 화면도 추적 | 기본 OFF. `/admin/*`, `/infrastructure/*`, `/access/authentication`으로 들어온 요청에는 넣지 않습니다. |
| 삽입 위치 | `head`(기본) 또는 `body` 끝 |
| 수집기 내부 HTTP 허용 | 기본 OFF. 다른 연동과 같은 규칙으로, 비 loopback HTTP 주소는 켜야만 저장됩니다. |

**Momento를 먼저 고르십시오.** Momento는 사내 자체 호스팅 수집기라 데이터가 밖으로 나가지
않는 유일한 선택지입니다. 같은 오리진 프록시를 켜 두면(기본) 브라우저는 jikim 오리진만 보므로
콘텐츠 보안 정책에 외부 출처가 아예 등장하지 않습니다. 프록시는 추적이 켜져 있고 제공자가
Momento일 때만 열리며, 그 밖에는 `/momento/*`가 `404`입니다. 프록시는 `GET`·`HEAD`·`POST`만
넘기고, 브라우저가 붙인 `Cookie`·`Authorization`·`X-Vault-Token`을 지운 뒤 전달하며, 수집기가
돌려준 `Set-Cookie`는 브라우저에 넘기지 않습니다. 본문은 64KB, 제한 시간은 10초입니다.

```html
<!-- 프록시를 켰을 때 화면에 들어가는 스니펫 (nonce는 요청마다 다릅니다) -->
...
jikim 화면은 `script-src 'self'`로 잠겨 있습니다. 스니펫을 그냥 붙이면 브라우저가 조용히
차단하고 관리자는 화면이 왜 비어 있는지 알 길이 없습니다. 그래서 추적을 켜면 서버가 다음을
함께 합니다.

1. 화면(SPA 셸)을 내려 줄 때마다 새 nonce를 만들어 스니펫의 **모든** `<script>` 태그에 붙이고,
   같은 nonce를 응답 헤더의 `script-src`에 `'nonce-…'`로 넣습니다. `'unsafe-inline'`은 쓰지
   않습니다 — 한 번 풀면 그 앱의 모든 인라인 스크립트가 함께 허용되고, 추적을 끈 뒤에도 정책이
   느슨한 채로 남기 때문입니다.
2. 제공자가 정해 주는 출처(GA4·GTM의 Google 주소, Matomo 주소, 프록시를 끈 Momento 주소)와
   직접 붙여넣은 스니펫 문자열 안의 `http(s)` 출처를 읽어 `script-src`·`connect-src`·`img-src`에
   더합니다.
3. 추적이 켜져 있는 동안에만 `report-uri /api/v1/tracking/csp-report`를 정책에 넣습니다.
   브라우저가 차단한 요청을 여기로 신고하면 서버가 **출처와 지시어**를 메모리에 기억합니다.
   같은 차단이 페이지마다 반복되므로 횟수가 아니라 서로 다른 출처가 중요하며, 100개를 넘으면
   가장 오래된 것부터 버립니다. 이 신고는 감사 로그에 남기지 않습니다.

탭 아래쪽 **정책이 차단한 출처** 표가 그 기록입니다. 막힌 출처 옆의 **허용**을 누르면 그
출처가 **추가 허용 출처**에 더해져 다음 화면 요청부터 정책에 반영됩니다. **기록 지우기**로
비운 뒤 화면을 다시 열어 아직 막히는 것이 있는지 확인하십시오. 이미 허용된 출처는 **허용됨**으로
표시됩니다.

추적을 끄면 정책은 이전 릴리스와 바이트 단위로 같은 값으로 돌아갑니다. `/api/*`, `/v1/*`,
`/mcp`, `/healthz`, `/readyz`, `/momento/*`처럼 브라우저가 그리지 않는 응답에는 스니펫을
넣지 않고 정책도 `default-src 'none'`으로 더 좁힙니다. 설정 저장소에 닿지 못하면 그 응답은
추적 없이(이전 정책으로) 내려가므로 로그인 화면이 함께 멈추지는 않습니다.

주의할 점 두 가지입니다. 화면은 SPA이므로 "관리 화면도 추적"이 꺼져 있어도 `/dashboard`로
들어온 뒤 화면 안에서 관리 메뉴로 이동하면 처음 실린 스니펫이 계속 동작합니다. 제외는 브라우저가
**처음 연 주소** 기준입니다. 그리고 로그인 화면에도 스니펫이 들어가므로 개인 식별 값을 보내는
스니펫은 붙이지 마십시오.

| 작업 | API | 역할 |
| --- | --- | --- |
| 차단 출처 목록 | `GET /api/v1/tracking/violations` | `admin` |
| 차단 기록 지우기 | `DELETE /api/v1/tracking/violations` | `admin` |
| 차단 출처를 허용 목록에 추가 | `POST /api/v1/tracking/violations/allow` (`{"origin":"https://…"}`) | `admin` |
| 브라우저 CSP 신고 수신 | `POST /api/v1/tracking/csp-report` | 인증 없음(브라우저가 보냄), 항상 `204` |

### 3.8 SMTP 메일 알림

**메일** 탭에서 사내 SMTP 릴레이로 이벤트 알림을 보냅니다. 기본값은 **꺼짐**이며, 새로 설치한
곳에서는 켜기 전까지 아무것도 달라지지 않습니다. 값은 다른 관리 설정과 같이 PostgreSQL의
`settings` 테이블(`mail` 키)에 저장되고, SMTP 비밀번호만 `mail_password` 키에 암호화해 따로
둡니다. 폐쇄망에서는 릴레이 주소로 사내 메일 서비스(`postra`)를 가리키면 알림이 밖으로 나가지
않습니다.

설정 이름은 사내 메일 알림 표준과 같습니다. 다른 서비스에서 배운 이름을 그대로 씁니다.

| 키 | 기본값 | 뜻 |
| --- | --- | --- |
| `mail.enabled` | `false` | 꺼짐이 기본. 관리자가 켭니다 |
| `mail.smtp_host` | — | 사내 릴레이 주소. 켜려면 필수 |
| `mail.smtp_port` | `25` | 사내 릴레이는 대개 25. 465면 `security`가 자동으로 `tls` |
| `mail.security` | `auto` | `auto`(서버가 STARTTLS를 알리면 씀) · `none` · `starttls`(필수) · `tls`(implicit) |
| `mail.skip_tls_verify` | `false` | 사내 인증서가 사설이고 CA를 신뢰 저장소에 넣을 수 없을 때만 |
| `mail.username` · `mail.password` | 빈 값 | 인증 없는 릴레이가 흔하므로 **선택 사항**. PLAIN·LOGIN·CRAM-MD5 중 서버가 알리는 것을 씁니다 |
| `mail.from_address` · `mail.from_name` | `jikim@<릴레이 주소>` · `jikim` | 보내는 사람 |
| `mail.base_url` | — | 메일 속 "바로 열기" 링크가 가리킬 이 앱의 주소. 비우면 링크를 넣지 않습니다 |
| `mail.timeout_seconds` | `10` | 연결·응답 제한 시간(1\~300) |
| `mail.notify_approval_request` | `true` | 승인 요청 → 검토자 |
| `mail.notify_approval_decision` | `true` | 승인·반려 결과 → 요청자 |
| `mail.notify_rotation_failed` | `true` | 회전 실패 → Secret 소유자(소유자가 없으면 관리자) |

값의 타입이 맞지 않으면 `400`으로 거부하고, `mail.enabled`를 켜면서 릴레이 주소가 없으면 저장
자체를 거부합니다. **비밀번호는 설정 API가 돌려주지 않습니다.** `GET /api/v1/settings`에는
`password_configured: true`만 실리고, 화면에는 "비밀번호 설정됨"으로만 보이며, 바꿀 때만 새
값을 받습니다. 서버 로그에도 남기지 않습니다.

#### 어떤 일이 메일로 가는가

보내는 이벤트는 세 가지입니다. 기준은 하나 — **이 메일이 오지 않으면 누군가 손해를 보거나
화면을 계속 새로고침한다**는 것입니다. 시크릿이 생성·변경됐다는 식의 "무언가 바뀜"은 메일로
보내지 않고 서명 Webhook(3.6절)에 남깁니다.

| 이벤트 | 받는 사람 | 왜 |
| --- | --- | --- |
| `approval.requested` | 검토 역할의 활성 계정 전원(`admin`, 검토 역할이 `manager`면 `manager`도) | 검토자가 모르면 요청자는 기다리기만 합니다 |
| `approval.decided` | 요청자 | 결과를 기다리며 승인 화면을 새로고침하지 않게 합니다 |
| `rotation.failed` | Secret 소유자(없으면 `admin` 전원) | 조용히 실패한 회전은 정책보다 오래된 자격증명이 됩니다 |

세 가지 규칙이 항상 적용됩니다. **자기가 한 일은 자기에게 보내지 않습니다**(요청자가 검토자
역할이어도 자기 요청 알림은 받지 않음). 같은 주소로는 한 통만 갑니다. 받는 사람의 주소는
사용자 계정의 이메일에서 가져오며, 메일 기능이 따로 명부를 갖지 않습니다 — 프로필에 이메일이
없는 계정은 조용히 건너뜁니다.

#### 요청을 막지 않는다

메일은 **배경에서** 보냅니다. 릴레이가 느리거나 죽어 있어도 승인 요청·회전 요청은 평소처럼
끝나고, 발송 실패는 그 요청의 실패가 아닙니다. 연결이 거부되면 2초 뒤 한 번 더 시도하고, 동시
발송 슬롯 16개가 모두 차면 그 건은 대기 없이 실패로 기록됩니다. 설정 저장소에 닿지 못해 발송
기록을 남길 수 없으면 보내지 않습니다 — 무엇이 나갔는지 말할 수 없는 메일은 보내지 않는 편이
낫기 때문입니다.

#### 시험 발송과 발송 기록

릴레이 설정은 한 번에 맞는 일이 드뭅니다. 설정을 **저장한 뒤** 탭 아래쪽 **테스트 메일
보내기**를 누르면 저장된 설정으로 실제 한 통을 보내고 결과(릴레이가 돌려준 오류 문구 포함)를
그 자리에서 보여 줍니다. 받는 사람을 비우면 내 프로필의 이메일로 갑니다. 저장하지 않은 변경이
있으면 단추가 잠깁니다.

**발송 기록**은 시도마다 남깁니다 — 언제, 어떤 이벤트로, 누구에게, 제목이 무엇이었고,
`sent`·`failed` 중 무엇이었는지, 몇 번 시도했고 실패했다면 릴레이가 뭐라고 했는지. "안 왔다"는
문의에는 이 표로 답합니다. **본문은 기록하지 않습니다.** 알림 기록이 그 자체로 유출 경로가 되지
않게 하기 위해서입니다.

| 작업 | API | 역할 |
| --- | --- | --- |
| 시험 발송 | `POST /api/v1/integrations/mail/test` (`{"recipient":"ops@corp.example"}`, 비우면 내 이메일) | `admin` |
| 발송 기록 조회 | `GET /api/v1/integrations/mail/deliveries?status=failed&limit=50&offset=0` | `admin`, `auditor` |

시험 발송은 메일이 꺼져 있으면 `400 mail_disabled`, 릴레이 주소가 없으면 `400 mail_not_configured`를
돌려주고, 릴레이가 거부하면 `200`에 `ok: false`와 이유를 실어 줍니다.
...
{"level":"INFO","msg":"jikim 시작","address":":8080","version":"v0.2.13"}
```
...
| 증상 | 확인할 곳 | 조치 |
| --- | --- | --- |
| `/readyz`가 `503` + `데이터베이스 연결을 확인할 수 없습니다` | PostgreSQL 도달성, DSN, TLS 모드, 계정 권한 | 저장소를 복구합니다. `v0.2.13`은 DB 장애를 자격증명 거부로 접지 않고 `500`으로 구분해 보고합니다. |
...
| 내부 HTTP 허용(OIDC·AI·Webhook·방문 추적 수집기) | 모두 OFF | 켜지 않습니다. TLS를 적용할 수 없는 신뢰된 내부 개발망에서만 예외로 씁니다. |
| 승인 워크플로 | 비활성 | 조직이 검토 절차를 요구하면 켜고 두 계정으로 시험합니다. |
| AI 연동 | 비활성 | 필요할 때만 켜고 승인된 내부 Gateway 주소만 씁니다. |
| 방문 추적 | 꺼짐 | 켠다면 Momento를 같은 오리진 프록시로 붙입니다. 외부 제공자는 데이터가 밖으로 나가므로 폐쇄망에서는 쓰지 않습니다. |
