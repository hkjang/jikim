...
이 문서는 jikim `v0.2.16` 관리 화면을 실제로 쓰는 사람을 위한 안내입니다. 화면 캡처는 모두
...
2. 관리자가 Keycloak SSO를 켰다면 입력란 아래에 **Keycloak SSO로 로그인** 버튼이 함께
   보입니다. 위 캡처는 SSO를 켜지 않은 상태라 버튼이 없습니다. 관리자가 **자동 로그인**까지
   켰다면 Keycloak에 이미 로그인한 상태로 jikim을 열었을 때 이 화면을 거치지 않고 바로 원래
   열려던 화면이 뜹니다. Keycloak 세션이 없거나 스스로 로그아웃한 뒤에는 평소처럼 로그인
   화면이 뜨며, 그때는 자동으로 다시 시도하지 않습니다.
3. 카드 아래쪽의 `jikim v0.2.16`이 관리자가 안내한 버전과 같은지 확인합니다. 다르면 접속
   주소를 잘못 찾았을 수 있습니다.
...
6. **시크릿 값** 칸에는 JSON 객체만 넣을 수 있습니다. `{ "username": "", "password": "" }`
   ...
   남지 않습니다.
7. 오른쪽 **회전 정책**의 **자동 회전 예약**은 후속 기능이라 켤 수 없습니다. `v0.2.16`은 상세
...
반려된 요청은 사유를 확인하고 새 요청으로 다시 제출합니다. `v0.2.16`의 시크릿 제출 화면에는
...
채팅 요청의 오류는 서버의 안내 메시지로 표시되며, 세션이 만료되면 다른 관리 기능과 동일한 재로그인 흐름으로 돌아갑니다.
...
- 관리자가 **MCP SSO(OAuth)** 를 켰다면 MCP 클라이언트에는 개인 토큰 대신 MCP 주소
  (`https://<서비스 주소>/mcp`) 하나만 넣어도 됩니다. 클라이언트가 Keycloak 로그인을 띄우고
  토큰을 받아 옵니다. 먼저 이 웹에 한 번 로그인해 둔 계정만 통과하며, 자세한 내용은
  [API 및 MCP 가이드](guides/api-guide.md)의 "키 없이 SSO로 연결하기"를 보십시오.
...
2. `v0.2.16`의 화면에는 이미 만든 시크릿의 Owner 표시나 애플리케이션 연결만 고치는 편집
