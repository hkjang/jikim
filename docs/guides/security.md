...
이 문서는 jikim `v0.2.21`의 보안 경계와 운영자가 추가로 구성해야 할 통제를 설명합니다. Secret 관리 서비스 자체가 단일 실패 지점이 되지 않도록 애플리케이션, 데이터베이스, 키, 네트워크와 운영자 권한을 함께 다룹니다.
...
v0.2.21 저장 암호화는 AES-256-GCM을 사용합니다. GCM nonce와 인증 태그로 암호문 변조를 탐지하며, 사용자 키와 Secret 버전은 암호문 형태로 PostgreSQL에 저장합니다.
...
v0.2.21 릴리스 자동화는 이미지 번들과 SHA-256을 생성하지만, 서명·SBOM·취약점 검사 도구가 별도 공급망 절차 없이 자동 제공된다고 가정하지 마십시오.
