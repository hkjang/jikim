...
이 문서는 jikim `v0.2.4` HTTP 인터페이스의 공통 계약을 설명합니다. 구현의 최종 기준은 해당 릴리스 소스와 자동 테스트이며, 이 문서에 없는 OpenBao API가 동작한다고 가정하면 안 됩니다.
...
v0.2.4은 OpenBao 전체 API 호환이나 99.9% 호환을 주장하지 않으며 OpenBao 2.6.1 differential suite도 아직 없습니다. [호환성 가이드](compatibility.md)를 먼저 확인하십시오.
