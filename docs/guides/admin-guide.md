...
이 문서는 jikim `v0.2.3` 서비스 관리자를 위한 운영 절차입니다. OS·컨테이너 부트스트랩과 일상적인 서비스 설정을 분리합니다.
...
Reverse Proxy 뒤에 배포한다면 **시스템 설정 → 보안**의 `신뢰 Reverse Proxy`에 Proxy 대역을 등록하십시오. 등록 전에는 감사 로그 IP와 로그인 실패 제한이 모두 Proxy 주소 기준으로 기록됩니다. 자세한 판별 규칙은 [보안 가이드](security.md)를 참조하십시오.
...
v0.2.3에서 설정할 수 있는 항목은 다음과 같습니다.
