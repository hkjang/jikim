...
이 절차는 `jikim:v0.2.4` 서비스 이미지를 `jikim-v0.2.4.tar.gz`로 반입하는 단일 노드 Docker 배포 프로파일입니다. PostgreSQL 설치와 백업은 운영 조직의 표준 절차를 따릅니다.
...
docker image inspect jikim:v0.2.4 --format '{{ index .Config.Labels "org.opencontainers.image.version" }}'
```

마지막 명령 결과는 `v0.2.4`이어야 합니다.
...
서비스 개방 전 로그인 화면과 프로필 메뉴의 버전이 `v0.2.4`인지 확인합니다.
